...
@section('content')
    <h1 class="h3 mb-3">Perhitungan Rekomendasi dengan ELECTRE I</h1>
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-5 col-md-3 pe-0 border-end">
                            <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist"
                                ...
                                <button class="nav-link text-start" id="v-pills-discordance-matriks-tab"
                                    ...
                                    Discordance</button>
                                <button class="nav-link text-start" id="v-pills-dominan-concordance-tab"
                                    data-bs-toggle="pill" data-bs-target="#v-pills-dominan-concordance" type="button"
                                    role="tab" aria-controls="v-pills-dominan-concordance" aria-selected="false">Dominan
                                    Concordance</button>
                                <button class="nav-link text-start" id="v-pills-dominan-discordance-tab"
                                    data-bs-toggle="pill" data-bs-target="#v-pills-dominan-discordance" type="button"
                                    role="tab" aria-controls="v-pills-dominan-discordance" aria-selected="false">Dominan
                                    Discordance</button>
                                <button class="nav-link text-start" id="v-pills-matriks-agregat-tab" data-bs-toggle="pill"
                                    data-bs-target="#v-pills-matriks-agregat" type="button" role="tab"
                                    aria-controls="v-pills-matriks-agregat" aria-selected="false">Matriks Agregat</button>
                                <button class="nav-link text-start" id="v-pills-ranking-tab" data-bs-toggle="pill"
                                    ...
                                    aria-controls="v-pills-ranking" aria-selected="false">Eliminasi</button>
                            </div>
                        </div>
                        <div class="col-7 col-md-9 ps-0">
                            <div class="tab-content" id="v-pills-tabContent">
                                ...
                                <div class="tab-pane fade" id="v-pills-dominan-concordance" role="tabpanel"
                                    aria-labelledby="v-pills-dominan-concordance-tab" tabindex="0">
                                    <div class="row ms-2">
                                        <div class="col-3">
                                                    ...
                                                    <tr>
                                                        ...
                                                        @for ($j = 1; $j <= $jumlahData; $j++)
                                                            @if ($i == $j)
                                                                <td>-</td>
                                                            @else
                                                                <td>{{ $concordanceDominanValue['c' . $i . '-' . $j] ?? '-' }}
                                                                </td>
                                                            @endif
                                                        @endfor
                                                    </tr>
                                ...
                                <div class="tab-pane fade" id="v-pills-dominan-discordance" role="tabpanel"
                                    aria-labelledby="v-pills-dominan-discordance-tab" tabindex="0">
                                    <div class="row ms-2">
                                        <div class="col-3">
                                                    ...
                                                    <tr>
                                                        ...
                                                        @for ($j = 1; $j <= $jumlahData; $j++)
                                                            @if ($i == $j)
                                                                <td>-</td>
                                                            @else
                                                                <td>{{ $discordanceDominanValue['d' . $i . '-' . $j] ?? '-' }}
                                                                </td>
                                                            @endif
                                                        @endfor
                                                    </tr>
                                ...
                                <div class="tab-pane fade" id="v-pills-matriks-agregat" role="tabpanel"
                                    aria-labelledby="v-pills-matriks-agregat-tab" tabindex="0">
                                    <div class="table-responsive">
                                ...
                                <div class="tab-pane fade" id="v-pills-ranking" role="tabpanel"
                                    ...
                                    <div class="alert alert-success d-flex flex-column gap-2 p-4 shadow-sm rounded-3 ms-3"
                                        role="alert">
                                        <div>
                                            <h5 class="alert-heading mb-1 fw-bold">🎯 Alternatif Terpilih</h5>
                                            <hr class="mt-1 mb-3">
                                        </div>

                                        <div>
                                            <p class="mb-1"><strong>Nama Sekolah:</strong>
                                                {{ $sekolahTerbaik['sekolah'] }}</p>
                                            <p class="mb-1"><strong>Wilayah:</strong> {{ $sekolahTerbaik['wilayah'] }}
                                            </p>
                                            <p class="mb-0"><strong>Perolehan Agregat:</strong>
                                                {{ $sekolahTerbaik['total_agregat'] }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    @push('script')
        <script>
            $(document).ready(function() {
                ...
                // Fungsi inisialisasi tiap tabel
                function initDataAwal() {
                    if (!$.fn.dataTable.isDataTable('#dataAwal')) {
                        $('#dataAwal').DataTable({
                            processing: false,
                            serverSide: true,
                            scrollX: true,
                ...
                function initMatriksKeputusan() {
                    if (!$.fn.dataTable.isDataTable('#matriksKeputusan')) {
                        $('#matriksKeputusan').DataTable({
                            processing: false,
                            serverSide: true,
                            scrollX: true,
                            deferRender: true,
                ...
                function initNormalisasi() {
                    if (!$.fn.dataTable.isDataTable('#normalisasi')) {
                ...
                function initBobotTernormalisasi() {
                    if (!$.fn.dataTable.isDataTable('#bobotTernormalisasi')) {
                        $('#bobotTernormalisasi').DataTable({
                            processing: false,
                            serverSide: true,
                            scrollX: true,
                            deferRender: true,
                ...
                function initConcordanceIndex() {
                    if (!$.fn.dataTable.isDataTable('#concordanceIndex')) {
                        $('#concordanceIndex').DataTable({
                            processing: false,
                            serverSide: true,
                            scrollX: true,
                            deferRender: true,
                ...
                function initDiscordanceIndex() {
                    if (!$.fn.dataTable.isDataTable('#discordanceIndex')) {
                        $('#discordanceIndex').DataTable({
                            processing: false,
                            serverSide: true,
                            scrollX: true,
                            deferRender: true,
                ...
                $('button[data-bs-toggle="pill"]').on('shown.bs.tab', function(e) {
                    ...
                    switch (target) {
                        ...
                        case '#v-pills-matriks-keputusan':
                            initMatriksKeputusan();
                            break;
                        case '#v-pills-normalisasi':
                            initNormalisasi();
                            break;
                        case '#v-pills-bobot-ternormalisasi':
                            initBobotTernormalisasi();
                            break;
                        case '#v-pills-concordance-index':
                            initConcordanceIndex();
                            break;
                        case '#v-pills-discordance-index':
                            initDiscordanceIndex();
                            break;
                    }
                });

                // Load tab pertama langsung (data awal)
                initDataAwal();
            });
        </script>
    @endpush
